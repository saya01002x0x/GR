...
export class CommentsService {
  ...
  async createComment(
    ...
    try {
      ...
      this.logger.error(
        `Failed to queue comment stats for ${artworkId}`,
        queueError,
      );
    }
  ...
  async deleteComment(userId: string, commentId: string) {
    ...
    const result: any[] = await this.prisma.$queryRaw`
        ...
        `;
    const totalToDelete = Number(
      (result as unknown as Array<{ count: number | string }>)[0]?.count || 1,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to queue comment delete stats for ${comment.artworkId}`,
        queueError,
      );
    }
