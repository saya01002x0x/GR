...
export class StatsProcessor extends WorkerHost {
  ...
  async process(job: Job<UpdateStatsJob>): Promise<any> {
    ...
    this.logger.log(
      `🔧 Processing ${type} stats for artwork ${artworkId} (delta: ${delta}, jobId: ${job.id})`,
    );
    ...
    try {
      ...
      if (type === 'view') {
        ...
        if (!redisCount) {
          this.logger.debug(
            `Skipping view stats for artwork ${artworkId} (no pending views in Redis)`,
          );
          return { success: true };
        }
      ...
      this.logger.debug(
        `Stats updated: artwork=${artworkId}, ${type} += ${delta}`,
      );
      return { success: true };
    } catch (error: any) {
