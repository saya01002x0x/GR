...
export class ViewService {
     ...
     */
    async recordView(artworkId: string, userId?: string, ip?: string): Promise<boolean> {
        ...
        try {
            ...
            const wasSet = await this.redis.set(lockKey, '1', 'EX', VIEW_LOCK_TTL, 'NX');

            if (!wasSet) {
                // Key already exists → spam protection, skip
                this.logger.debug(`View blocked (spam): artwork=${artworkId}, identity=${identity}`);
                return false;
            }
            ...
            this.logger.debug(`View recorded: artwork=${artworkId}, identity=${identity}`);
            return true;
        } catch (error) {
            this.logger.error(`Failed to record view for artwork ${artworkId}`, error);
            return false;
        }
    }
}
